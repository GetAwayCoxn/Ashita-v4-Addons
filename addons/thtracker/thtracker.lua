...
ashita.events.register('text_in', 'text_in_cb', function(e)
    ...
    if not area or Towns:contains(area) or player:GetIsZoning() ~= 0 then
        mobs = T{}
		return
	end
    ...
    elseif e.message:contains(me .. ' scores a critical') or (e.message:contains(me) and e.message:contains('hit')) or e.message:contains(me .. '\'s ranged attack') or (e.message:contains(me) and e.message:contains('RA')) then
        local index = AshitaCore:GetMemoryManager():GetTarget():GetTargetIndex(0)
        if index == nil then return end
        if AshitaCore:GetMemoryManager():GetEntity():GetType(index) ~= 2 then return end
	    local target = GetEntity(index)
        ...
        if player:GetMainJob(0) == 6 then
            local mLv = player:GetMainJobLevel(0)
            if mLv >= 90 then
                count = 3
            elseif mLv >= 45 then
                count = 2
        ...
        elseif player:GetSubJob(0) == 6 then
            local sLv = player:GetSubJobLevel(0)
            if sLv >= 45 then
                count = 2
            elseif sLv >= 15 then
                count = 1
